...
class CardGridProfileRepository @Inject() (
  ...
  /**
    ...
    */
  def read(id: String): Future[CardGridProfileData] =
    Future {
  ...
  def readFromName(
      name: String,
      user: User
  ): Future[Option[CardGridProfileData]] = {
  ...
  /**
    ...
    */
  def getProfileIdFromName(name: String, user: User): Future[Option[String]] =
    Future {
      import anorm.SqlParser._
      db.withConnection { implicit c =>
        SQL(
          "SELECT id FROM cardGridProfiles WHERE name = {name} AND userId = {userId}"
        )
  ...
  /**
    ...
    */
  def listNames(user: User): Future[List[String]] =
    Future {
      import anorm.SqlParser._
      db.withConnection { implicit c =>
...
class CardGridProfileCreator(
  ...
  def createIncludeTags(): Unit =
    input.config.includeTags.getOrElse(List()).foreach { tag =>
      SQL(
        """INSERT INTO cardGridConfigIncludeTags(configId, tag) VALUES ({configId}, {tag})"""
      )
        .on("configId" -> configId, "tag" -> tag)
        .executeInsert()
    }

  def createExcludeTags(): Unit =
    input.config.excludeTags.map { tags =>
      tags.foreach { tag =>
        SQL(
          """INSERT INTO cardGridConfigExcludeTags(configId, tag) VALUES ({configId}, {tag})"""
        )
          .on("configId" -> configId, "tag" -> tag)
          .executeInsert()
      }
