...
class UserTokenRepository @Inject() (
  ...
  private val userTokenParser: RowParser[UserToken] =
    Macro.namedParser[UserToken](
      ColumnNaming(x =>
        x match {
          case "user" => "userId"
          case x      => x
  ...
  /**
    ...
    */
  implicit def columnTouser: Column[User] =
    IdColumnToUserConverter(userRepository).convert

  /**
    * Adds and returns a userToken.
    */
  def add(userToken: UserToken): Future[UserToken] =
    Future {
  ...
  /**
    ...
    */
  def findByTokenValue(tokenValue: String): Future[Option[UserToken]] =
    Future {
