...
class OneTimePasswordProvider @Inject() (
  ...
  def authenticate(
  ...
  ): Future[Try[LoginInfo]] = {
    logger.info(
      "Authenticating LoginInfo " + loginInfo + " and password " + oneTimePassword
    )
    oneTimePassInfoRepository.find[OneTimePasswordInfo](loginInfo).map {
      ...
      case Some(oneTimePasswordInfo: OneTimePasswordInfo) => {
        ...
        oneTimePassInfoRepository
          .update(loginInfo, oneTimePasswordInfo.copy(hasBeenUsed = true))
        Success(loginInfo)
      }
    }
  }
  ...
  /**
    ...
    */
  def authenticate(
      credentials: OneTimePasswordCredentials
  ): Future[Try[LoginInfo]] = {
