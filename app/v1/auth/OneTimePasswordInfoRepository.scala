...
class OneTimePasswordInfoRepository @Inject() (val db: Database)(implicit
  ...
  def find[T <: AuthInfo](
      loginInfo: LoginInfo
  )(implicit tag: ClassTag[T]): Future[Option[T]] = {
    Future {
  ...
  def add[T <: AuthInfo](loginInfo: LoginInfo, authInfo: T): Future[T] =
    Future {
      val oneTimePasswordInfo: OneTimePasswordInfo =
        authInfo.asInstanceOf[OneTimePasswordInfo]
      db.withConnection { implicit e =>
        logger.info(
          "Adding OneTimePasswordInfo " + authInfo + " for " + loginInfo
        )
        SQL(
  ...
  def update[T <: AuthInfo](loginInfo: LoginInfo, authInfo: T): Future[T] =
    Future {
      ...
      db.withConnection { implicit e =>
        logger.info(
          "Updating OneTimePasswordInfo for " + loginInfo + " to " + authInfo
        )
        SQL(
