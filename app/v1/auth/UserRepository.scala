...
class UserRepository @Inject()(
  ...
  def add(user: User): Future[User] = Future {
      ...
      value match {
        case id: String => {
          val futureUser = userRepository.findById(id).map {
            ...
            case None       => throw RelatedObjectDoesNotExist("Could not find user with id " + id)
          }
          Await.result(futureUser, Duration.Inf)
        }
        case _          => Left(TypeDoesNotMatch(s"Cannot convert $value: ${value.asInstanceOf[AnyRef].getClass} to User for column $qualified"))
      }
    }
  }
}
...
object IdColumnToUserConverter {
  def apply(u: UserRepository)(implicit ec: ExecutionContext) = new IdColumnToUserConverter(u)
}
