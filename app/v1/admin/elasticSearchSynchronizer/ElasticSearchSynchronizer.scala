...
class ElasticSearchSynchornizer @Inject()(
  ...
  def deleteStaleEntries() = {
    esClient.getAllIds().flatMap { esIds =>
      logger.info(s"Ids in ES: ${esIds}")
      dbReader.getAllIds().flatMap { dbIds =>
...
class DatabaseReader @Inject()(db: Database)(implicit val ec: ExecutionContext) {
  ...
  /**
    ...
    */
  def getAllIds(): Future[Set[String]] = Future { db.withConnection { implicit c =>
...
/**
  ...
  */
class SyncronizerElasticClient @Inject()(
  ...
  /**
    ...
    */
  def idsScroller = new IdsScroller(index, elasticClient, requestSize, scrollKeepAlive)
  ...
  def send(row: DatabaseRow): Future[Unit] = {
    ...
    elasticClient.execute {
      indexInto(index).id(row.id).fields(
        "title" -> row.title,
        "body" -> row.body,
      ...
      case Success(value: RequestFailure) => logger.error(s"Request failed! ${value}")
      case Success(value: RequestSuccess[_]) => logger.info(s"Success: ${value}")
    } map (_ => ())
  }
  ...
  /**
    * Deletes an id.
    */
  def delete(id: String): Future[Unit] = elasticClient.execute {
    logger.info(s"Deleting: ${id}")
    deleteById(index, id)
  } map {
    case value: RequestFailure => throw new Exception(s"Failed to delete ${id}")
...
class IdsScroller(
  ...
  /**
    ...
    */
  def isFinished(ids: List[String], results: SearchResponse) = ids.length == results.totalHits

  /**
    * Prepares the next request.
    */
  def nextReq(resp: SearchResponse) = elasticClient.execute {
  ...
  ): Future[List[String]] = request.map {
  ...
  def run(): Future[List[String]] = {
    val query = search(index).matchAllQuery().scroll(scrollKeepAlive).size(requestSize)
