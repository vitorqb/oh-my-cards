...
case class CardFormInput(
  ...
  def toCreateData(): CardCreateData =
    new CardCreateData(getTitle(), getBody(), getTags())
...
case class CardListRequestInput(
  ...
  def tagsList: List[String] =
    tags.map(StringUtils.splitByComma).getOrElse(List())
  def tagsNotList: List[String] =
    tagsNot.map(StringUtils.splitByComma).getOrElse(List())
...
class CardController @Inject() (
    ...
    silhouette.SecuredAction.async { implicit request =>
      ...
      CardFormInput.form
        ...
        .fold(
          ...
          cardFormInput =>
            resourceHandler.create(cardFormInput, request.identity) map {
        ...
        case Good(id) =>
          CardFormInput.form
            ...
            .fold(
              ...
              cardFormInput =>
                resourceHandler
                  .update(id, cardFormInput, request.identity) map { card =>
                  Ok(Json.toJson(card))
                } recover { e =>
                  handleError(e)
                }
  ...
  def delete(id: String) =
    silhouette.SecuredAction.async { implicit request =>
      ...
      parseUUID(id) match {
        ...
        case Good(x) =>
          resourceHandler.delete(x, request.identity).map { _ =>
            {
  ...
  /**
    * Handles errors.
    */
  private def handleError(error: Throwable): Result =
    error match {
      case userErr: CardRepositoryUserException => {
        ...
        userErr match {
          ...
          case TagsFilterMiniLangSyntaxError(m, _) =>
            BadRequest(Json.obj("message" -> m))
        }
      }
      case _ => {
