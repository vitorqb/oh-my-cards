  ...
  lazy val bucketName = {
    ...
    if (config.bucketId != allowed.bucketId)
      throw new RuntimeException(
        "B2FileRepository does not has access to the bucket."
      )
    allowed.bucketName
  }

  override def store(key: String, file: File): Future[Unit] =
    Future {
      logger.info(f"Started upload with key ${key}.")
      val contentType = Files.probeContentType(file.toPath())
      val source = B2FileContentSource.builder(file).build()
      val req = B2UploadFileRequest
        .builder(config.bucketId, key, contentType, source)
        .build()
...
/**
  ...
  */
class MockFileRepository()(implicit val ec: ExecutionContext)
    extends FileRepositoryLike {
