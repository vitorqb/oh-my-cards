...
class CardElasticClientFunctionalSpec
  ...
  val cardContext1 =
    CardCreationContext(user, DateTime.parse("2020-01-01T00:00:00Z"), "id1", 1)
  val cardData1 = cardContext1.genCardData(cardCreateData1)

  val cardCreateData2 = CardCreateData("FOO BYE", "", List("T1", "T3"))
  val cardContext2 =
    CardCreationContext(user, DateTime.parse("2020-01-02T00:00:00Z"), "id2", 2)
  val cardData2 = cardContext2.genCardData(cardCreateData2)

  val cardCreateData3 = CardCreateData("BAR", "FOO BAR", List())
  val cardContext3 =
    CardCreationContext(user, DateTime.parse("2020-01-03T00:00:00Z"), "id3", 3)
  val cardData3 = cardContext3.genCardData(cardCreateData3)

  val cardCreateData4 =
    CardCreateData("BYE BYE DUDE", "I SAID BYE", List("T1", "T2", "T4"))
  val cardContext4 =
    CardCreationContext(user, DateTime.parse("2020-01-04T00:00:00Z"), "id4", 4)
  val cardData4 = cardContext4.genCardData(cardCreateData4)

  val allInputs =
    Seq(cardCreateData1, cardCreateData2, cardCreateData3, cardCreateData4)
  ...
  case class TestContext(
    ...
    override implicit def patienceConfig =
      new PatienceConfig(Span(3000, Millis))

    def saveCardsToDb(): Unit =
      allFixtures.foreach(x => saveCardToDb(x._1, x._2))
    def saveCardToDb(
        data: CardCreateData,
  ...
  "Functional Tests for ES card client" should {
    ...
    "Create and delete a card" taggedAs (FunctionalTestsTag) in testContext {
      c =>
        ...
        resultAfter.hits.hits.head.sourceAsMap("tags") mustEqual cardData.tags
          .map(_.toLowerCase())
        resultAfter.hits.hits.head
          .sourceAsMap("createdAt")
          .asInstanceOf[String] mustEqual "2020-01-04T00:00:00.000Z"
        resultAfter.hits.hits.head
          .sourceAsMap("updatedAt")
          .asInstanceOf[String] mustEqual "2020-01-04T00:00:00.000Z"
    ...
    "Filter cards by userId" should {
      ...
      "find no match if user has no cards" taggedAs (FunctionalTestsTag) in testContext {
        c =>
          c.saveCardsToDb()
          refreshIdx()
          val result = c.cardElasticClient
            .findIds(cardListData.copy(userId = "NOT_AN_USER_ID"))
            .futureValue
          result.ids mustEqual List()
          result.countOfItems mustEqual 0
      }

      "find all ids if user has cards" taggedAs (FunctionalTestsTag) in testContext {
        c =>
          c.saveCardsToDb()
          refreshIdx()
  ...
  "Do pagination" should {
    ...
    "Find first page with 2 cards" taggedAs (FunctionalTestsTag) in testContext {
      c =>
        c.saveCardsToDb()
        refreshIdx()
        val result = c.cardElasticClient
          .findIds(cardListData.copy(pageSize = 2))
          .futureValue
        result.ids mustEqual List(cardContext4.id, cardContext3.id)
        result.countOfItems mustEqual 4
    }

    "Find second page with 2 cards" taggedAs (FunctionalTestsTag) in testContext {
      c =>
        c.saveCardsToDb()
        refreshIdx()
        val result = c.cardElasticClient
          .findIds(cardListData.copy(pageSize = 2, page = 2))
          .futureValue
        result.ids mustEqual List(cardContext2.id, cardContext1.id)
        result.countOfItems mustEqual 4
    }

    "Find last page with no cards" taggedAs (FunctionalTestsTag) in testContext {
      c =>
        c.saveCardsToDb()
        refreshIdx()
        val result = c.cardElasticClient
          .findIds(cardListData.copy(pageSize = 2, page = 3))
          .futureValue
        result.ids mustEqual List()
        result.countOfItems mustEqual 4
    }
  ...
  "Filter cards by search term" should {
    ...
    "match more than one" taggedAs (FunctionalTestsTag) in testContext { c =>
      ...
      refreshIdx()
      val result = c.cardElasticClient
        .findIds(cardListData.copy(searchTerm = Some("fOo")))
        .futureValue
    ...
    "match one by body" taggedAs (FunctionalTestsTag) in testContext { c =>
      ...
      refreshIdx()
      val result = c.cardElasticClient
        .findIds(cardListData.copy(searchTerm = Some("I SAID")))
        .futureValue
    ...
    "match one by title" taggedAs (FunctionalTestsTag) in testContext { c =>
      ...
      refreshIdx()
      val result = c.cardElasticClient
        .findIds(cardListData.copy(searchTerm = Some("HELLO")))
        .futureValue
    ...
    "match zero" taggedAs (FunctionalTestsTag) in testContext { c =>
      ...
      refreshIdx()
      val result = c.cardElasticClient
        .findIds(cardListData.copy(searchTerm = Some("&*@^!")))
        .futureValue
      result.ids mustEqual List()
      result.countOfItems mustEqual 0
    }
  ...
  "Filter cards by tag minilang query" should {
    ...
    "match more than one" taggedAs (FunctionalTestsTag) in testContext { c =>
      ...
      val result = c.cardElasticClient
        .findIds(cardListData.copy(query = query))
        .futureValue
      result.ids mustEqual List(
        cardContext4.id,
        cardContext2.id,
    ...
    "match one" taggedAs (FunctionalTestsTag) in testContext { c =>
      ...
      refreshIdx()
      val query = Some(
        """((tags CONTAINS 'T1') AND (tags CONTAINS 'T2') AND (tags CONTAINS 'T4'))"""
      )
      val result = c.cardElasticClient
        .findIds(cardListData.copy(query = query))
        .futureValue
  ...
  "Sorting" should {
    ...
    "if the score is the same, sort by createdAt" taggedAs (FunctionalTestsTag) in testContext {
      c =>
    ...
    "if the score is not the same, sort by score" taggedAs (FunctionalTestsTag) in testContext {
      c =>
        val cardInput_1 = cardCreateData1.copy(title = searchTerm)
        val cardContext_1 = cardContext1.copy(id = "1", now = date1, ref = 1)

        val cardInput_2 = cardCreateData1.copy(title =
  ...
  "All together" should {
    ...
    "tags, pagination and body match" taggedAs (FunctionalTestsTag) in testContext {
      c =>
        c.saveCardsToDb()
        refreshIdx()
        val query = Some(
        ...
        val resultOne = c.cardElasticClient
          .findIds(
            cardListData.copy(
              page = 1,
              pageSize = 1,
        ...
        val resultTwo = c.cardElasticClient
          .findIds(
            cardListData.copy(
              page = 2,
              pageSize = 1,
        ...
        val resultThree = c.cardElasticClient
          .findIds(
            cardListData.copy(
              page = 1,
              pageSize = 2,
  ...
  "Error handling" should {
    ...
    "Returns failed future if minilang has parsing error" taggedAs (FunctionalTestsTag) in testContext {
      c =>
        val query = Some("(foo)")
