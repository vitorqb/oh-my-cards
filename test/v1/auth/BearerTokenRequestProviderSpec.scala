...
class BearerTokenRequestProviderSpec
  ...
  "authenticate" should {
    ...
    val validUserToken =
      UserToken(user, tokenValue, clock.now.plusMinutes(60), false)
    val invalidUserToken =
      validUserToken.copy(expirationDateTime = clock.now.minusMinutes(60))
    ...
    "Bring authenticated user if finds valid token" in {
      ...
      when(headers.get("Authorization"))
        .thenReturn(Some("Bearer " + tokenValue))

      val request = mock[Request[AnyContent]]
      when(request.headers).thenReturn(headers)

      val bearerTokenProvider =
        new BearerTokenRequestProvider(userTokenRepositoryMock, clock)
      val result = bearerTokenProvider.authenticate(request)
      result.futureValue mustEqual Some(
        LoginInfo(BearerTokenRequestProvider.ID, user.email)
      )
    }
  ...
  "extractTokenValue" should {
    ...
    "Extract valid value" in {
      ...
      BearerTokenRequestProvider.extractTokenValue(request) mustEqual Some(
        "foobarbaz"
      )
    }
